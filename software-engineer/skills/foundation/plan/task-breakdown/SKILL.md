...
2. **Smallest verifiable unit** — a task is done when there is something demonstrable (a passing test, a rendered page, a merged PR)
...
- What is the user-visible outcome? (feature, fix, improvement)
...
- What is the smallest thing we can ship and call "done"?
...
| **Migration** | Old to new, kept running | "Migrate payments from Stripe to LemonSqueezy" |
...
Task breakdown is a communication tool first, a planning tool second. A well-broken list surfaces disagreements about scope and approach before code is written — that is the real value. If the team argues about tasks, you have found ambiguity that would have been a merge conflict later.

## Resources

See `resources/estimation-guide.md` for techniques to size tasks reliably.
